...
impl<'c> SwigWallet<'c> {
    ...
    pub fn sign_v2(
        ...
        let sign_ix = self.get_sign_v2_instructions(inner_instructions)?;
    ...
    /// Gets the SignV2 instructions for the provided inner instructions
    ///
    /// # Arguments
    /// * `inner_instructions` - Vector of instructions to sign
    /// # Returns
    /// Returns a `Result` containing the sign v2 instructions or a `SwigError`
    pub fn get_sign_v2_instructions(
        &mut self,
        inner_instructions: Vec<Instruction>,
    ) -> Result<Vec<Instruction>, SwigError> {
        let sign_ix = self
